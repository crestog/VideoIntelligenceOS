"""
VIOS GraphRAG Prompt Suite — Pure-Data Module

All prompt templates for the GraphRAG pipeline (entity/relationship extraction,
summarization, RAG response generation, multi-choice QA, keyword extraction,
visual/entity retrieval rewrites, and segment filtering).

These are extracted verbatim from the notebook's PROMPTS dict and kept here
as a standalone module so any worker can import them without pulling in
GPU or model dependencies.
"""
...
("relationship"{tuple_delimiter}"Taylor"{tuple_delimiter}"Jordan"{tuple_delimiter}"Taylor and interact directly regarding the device, leading to a moment of mutual respect and an uneasy truce."{tuple_delimiter}8){record_delimiter}
